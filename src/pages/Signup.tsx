...
const Signup = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      // Only navigate if we're still on the signup page
      ...
      if (location.pathname.startsWith('/signup')) {
        ...
        if (role === 'owner') {
          if (userRole === 'owner') {
            // User is already owner, redirect to home page
            navigate('/', { replace: true });
          } else {
            // User exists but not as owner, redirect to login
            navigate(`/login?role=owner&next=/`, { replace: true });
          }
        } else {
          // Student signup, redirect to home page
          navigate('/', { replace: true });
        }
      }
    }
      ...
      <div className="flex-1 flex items-center justify-center p-8 overflow-y-auto">
        <div className="w-full max-w-md py-8">
          ...
          <div className="flex p-1 bg-secondary rounded-xl mb-8">
            <Link
              to={`/signup?role=student${next !== '/' ? `&next=${next}` : ''}`}
            ...
            <Link
              to={`/signup?role=owner${next !== '/' ? `&next=${next}` : ''}`}
              className={`flex-1 py-2.5 text-center rounded-lg text-sm font-medium transition-all ${
                role === 'owner'
                  ? 'bg-surface text-foreground shadow-soft'
                  : 'text-muted-foreground hover:text-foreground'
          ...
          <p className="text-center text-sm text-muted-foreground mt-8">
            ...
            <Link
              to={`/login?role=${role}${next !== '/' ? `&next=${next}` : ''}`}
      ...
      <div className="hidden lg:flex flex-1 bg-primary relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center p-12 text-primary-foreground">
          ...
          <p className="text-primary-foreground/80 text-lg">
            {role === 'owner'
              ? 'List your properties and connect with students looking for quality accommodation near their colleges.'
              : 'Create your account to save favorite properties, book visits, and find your perfect student accommodation.'}
          </p>
        </div>
      </div>
    </div>
